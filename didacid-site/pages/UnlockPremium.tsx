...
const UnlockPremium: React.FC = () => {
    const navigate = useNavigate();
    const { startPremiumSession } = useAuth();
    const plan = pricingPlans.find(p => p.id === 'premium-ia');
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (Object.keys(validationErrors).length === 0) {
            ...
            setTimeout(() => {
                console.log("One-time payment processed for:", formData.name);
                startPremiumSession(formData.name);
    ...
    return (
        <>
            <PageMeta
                title="Acheter une Résolution IA Premium - Didacid"
                description="Débloquez une session de support premium avec l'IA experte pour un paiement unique."
            />
            <div className="bg-background py-12 sm:py-20 animate-fadeIn">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="max-w-4xl mx-auto grid lg:grid-cols-2 gap-12">
                         <div className="lg:col-span-1">
                            <h1 className="text-3xl font-extrabold text-text-primary">Acheter une Résolution</h1>
                            <p className="mt-4 text-text-secondary">
                                Accédez à une session de support illimitée avec l'IA experte pour résoudre votre problème actuel.
                            </p>
                            <div className="bg-surface p-8 rounded-lg shadow-lg mt-8">
                                <form onSubmit={handleSubmit} className="space-y-4">
                                    ...
                                    <button type="submit" disabled={formState === 'submitting'} className="w-full inline-flex justify-center items-center gap-2 bg-primary text-white font-semibold py-3 px-8 rounded-full hover:bg-opacity-80 transition-all disabled:bg-gray-500">
                                        {formState === 'submitting' ? 'Traitement...' : `Payer ${plan.price} et Démarrer`}
                                        <Lock size={20} />
                                    </button>
                                </form>
                            </div>
                        ...
                        <div className="lg:col-span-1">
                             <div className="bg-surface p-8 rounded-lg shadow-lg sticky top-24">
                               <h2 className="text-2xl font-bold text-text-primary mb-6">Récapitulatif de votre achat</h2>
                               ...
                               <div className="flex justify-between items-center font-bold text-lg">
                                   <span>Total (paiement unique)</span>
